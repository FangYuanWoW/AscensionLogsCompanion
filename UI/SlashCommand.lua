...
SlashCmdList["ALC"] = function(msg)
    ...
    elseif cmd == "c1" or cmd == "c2" then
        -- Codec-overhaul test gate: c1 = raw 8-bit transport, c2 = c1 + D1
        -- dictionary. Sets ALC_Config.ci_codec and republishes in one step.
        _G.ALC_Config = _G.ALC_Config or {}
        local sub = parts[2]
        local haveDict = ALC.Core.Dictionaries and ALC.Core.Dictionaries.D1
        if sub == "on" then
            if not ALC.Core.Transport8 then
                L.warn("Transport8 NOT loaded (check .toc / reload) - staying on base64")
            elseif cmd == "c2" and not haveDict then
                L.warn("D1 dictionary NOT loaded (check .toc / reload) - use /alc c1 on instead")
            else
                ALC_Config.ci_codec = cmd
                L.info(cmd .. " transport ON ([[ALC_CI_v2_" .. cmd .. "_...). Republishing own CI...")
                if ALC.Capture.SnapshotPipeline then ALC.Capture.SnapshotPipeline.forcePublish() end
            end
        elseif sub == "off" then
            ALC_Config.ci_codec = "legacy"
            L.info(cmd .. " OFF (legacy base64).")
        else
            L.info("CI codec is " .. (ALC_Config.ci_codec or "legacy")
                .. " (Transport8 " .. (ALC.Core.Transport8 and "loaded" or "MISSING")
                .. ", D1 " .. (haveDict and "loaded" or "missing")
                .. "). Use /alc c1|c2 on|off")
        end

    elseif cmd == "frame" then
        -- Phase 4 test gate: bundle CI/TS/PP into [[ALC_F_v1_c2_...]] frames.
        _G.ALC_Config = _G.ALC_Config or {}
        local sub = parts[2]
        local haveD1 = ALC.Core.Dictionaries and ALC.Core.Dictionaries.D1
        if sub == "on" then
            if not (ALC.Core.Frame and haveD1) then
                L.warn("Frame/D1 not loaded (check .toc / restart client)")
            else
                ALC_Config.frame_codec = "c2"
                L.info("frame codec ON. CI/TS/PP -> [[ALC_F_v1_c2_...]] frames; KS stays priority. Republishing...")
                if ALC.Capture.SnapshotPipeline then ALC.Capture.SnapshotPipeline.forcePublish() end
                if ALC.Capture.FrameBuilder then ALC.Capture.FrameBuilder.flush() end
            end
        elseif sub == "off" then
            ALC_Config.frame_codec = nil
            L.info("frame codec OFF (legacy per-family chunks).")
        else
            L.info("frame codec is " .. (ALC_Config.frame_codec or "off")
                .. " (Frame " .. (ALC.Core.Frame and "loaded" or "MISSING")
                .. ", D1 " .. (haveD1 and "loaded" or "missing") .. "). Use /alc frame on|off")
        end

    elseif cmd == "c2check" then
        -- Diagnostic: exercise the LibDeflate dictionary path directly and print
        -- exactly where it fails (vs the silent c2->c1 degrade in the pipeline).
        local D = ALC.Core.Dictionaries and ALC.Core.Dictionaries.D1
        if not D then L.warn("c2check: ALC.Core.Dictionaries.D1 missing"); return end
        L.info("c2check: D1 declared size=" .. tostring(D.size) .. " actual #bytes=" .. #D.bytes)
        local deflate = _G.LibStub and _G.LibStub:GetLibrary("LibDeflate", true)
        if not deflate then L.warn("c2check: LibDeflate not found via LibStub"); return end
        local runAdler = deflate.Adler32 and deflate:Adler32(D.bytes)
        L.info("c2check: runtime Adler32=" .. tostring(runAdler) .. " embedded=" .. tostring(D.adler32))
        local okD, dictOrErr = pcall(function() return deflate:CreateDictionary(D.bytes, #D.bytes, runAdler) end)
        if not okD then L.warn("c2check: CreateDictionary FAILED: " .. tostring(dictOrErr)); return end
        L.info("c2check: CreateDictionary OK")
        local okC, outOrErr = pcall(function() return deflate:CompressDeflateWithDict("hello world hello world", dictOrErr, { level = 5 }) end)
        if not okC then L.warn("c2check: CompressDeflateWithDict FAILED: " .. tostring(outOrErr)); return end
        L.info("c2check: CompressDeflateWithDict OK (" .. #outOrErr .. "B)")
        local sc = ALC.Core.Serialize
        local ci = _G.ALC_LocalState and _G.ALC_LocalState.last_own_ci
        if ci and sc.serializeCIWithDict then
            local d = sc.serializeCIWithDict(ci)
            local p = sc.serializeCI(ci)
            L.info("c2check: CI plain=" .. (p and #p or "nil") .. "B dict=" .. (d and #d or "nil") .. "B")
        end
        L.info("c2check: PASS - dict path works; /alc c2 on should now emit real c2 chunks")
